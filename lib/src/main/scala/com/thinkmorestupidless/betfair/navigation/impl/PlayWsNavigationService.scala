...
import com.thinkmorestupidless.betfair.navigation.domain.{Menu, NavigationService}
...
final class PlayWsNavigationService(config: BetfairConfig)(implicit system: ActorSystem) extends NavigationService with JsonCodecs {

  private implicit val ec = system.dispatcher
  private val wsClient = StandaloneAhcWSClient()

  override def menu()(session: BetfairSession): Future[Menu] =
    wsClient.url(config.navigation.uri.value)
