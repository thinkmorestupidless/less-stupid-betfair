...
class PlayWsBetfairExchangeService(config: BetfairConfig)(implicit system: ActorSystem) extends BetfairExchangeService {
  ...
  override def listCompetitions(filter: MarketFilter)(implicit
      session: BetfairSession
  ): Future[List[CompetitionResult]] = ???
  ...
  override def listEventTypes(
      filter: MarketFilter
  )(implicit session: BetfairSession): Future[List[BetfairExchangeService.EventTypeResponse]] =
    wsClient
      .url(config.exchange.uris.listEventTypes.value)
      .addHttpHeaders(
  ...
  override def listEvents(filter: MarketFilter)(implicit
      session: BetfairSession
  ): Future[Set[BetfairExchangeService.EventResponse]] = ???

  override def listMarketCatalogue(listMarketCatalogue: BetfairExchangeService.ListMarketCatalogue)(implicit
      session: BetfairSession
  ): Future[List[MarketCatalogue]] = ???

  override def listMarketBook(listMarketBook: BetfairExchangeService.ListMarketBook)(implicit
      session: BetfairSession
