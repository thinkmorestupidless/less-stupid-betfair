...
object ChannelFilterFlow {
  ...
  type ChannelFilterFlow = BidiFlow[
    OutgoingBetfairSocketMessage,
    OutgoingBetfairSocketMessage,
    IncomingBetfairSocketMessage,
    IncomingBetfairSocketMessage,
  ...
  def apply(channelId: ChannelId, channelsService: ChannelsService)(implicit
      ec: ExecutionContext
  ): ChannelFilterFlow = {
    val outgoing = Flow[OutgoingBetfairSocketMessage].mapAsync(1) { elem =>
      val future = elem match {
        case MarketSubscription(_, Some(marketFilter)) =>
          channelsService.updateMarketFilterForChannel(channelId, marketFilter)
